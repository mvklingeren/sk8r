...
export function createWebSocketServer(): WebSocketServer {
	...
	wss.on('connection', (ws: WebSocket, request: IncomingMessage) => {
		...
		handleExecConnection(ws, namespace, podName, container, command);
	});
...
async function handleExecConnection(
	...
	let k8sConnection: { stdin: stream.Writable | null; close: () => void } = {
		stdin: null,
		close: () => {}
	};
	...
	try {
		...
		for (const shell of shellCommands) {
			try {
				...
				console.log(`[WebSocket] Shell ${shell} failed:`, (err as Error).message);
			}
		}
